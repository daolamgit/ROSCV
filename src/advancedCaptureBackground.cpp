...
class advancedCaptureBackground
{
  ros::NodeHandle nh_;
  ros::NodeHandle n;
 ros::Publisher pub ;
...
public:
 advancedCaptureBackground(const char* _camera)
    : it_(nh_)
  {
	camera = _camera;
...
//     cout << topic << endl;
     ...
     image_sub_ = it_.subscribe( imageTopic, 1, &advancedCaptureBackground::imageCb, this);
     image_pub_= it_.advertise("outLeft",1);
 
 
  }
 
  ~advancedCaptureBackground()
  {
...
int main(int argc, char** argv)
{
  ...
  if (ros::names::remap( "image") == "image") 
	{
		ROS_WARN( "command: \t$ ./advancedCaptureBackground image:=<image topic> [transport]");

	}	
  advancedCaptureBackground cB (argv[1]); //( (argc >1 ) ? argv[1] : "raw");
  ros::spin();
