...
static const char WINDOW[] = "advancedSubtractBackground Image window";
const float maxImageWidth = 1280;

const double Zo = -.91 +0.71+ .02; //base to the ground
const double pi = 3.1416;

//subtract Image: make it a member because it is processed by several function
//Mat subtractImage;

class advancedSubtractBackground
{
  ros::NodeHandle nh_;
 ...
 ros::Subscriber sub; //for subscribe data
  ...
  //for object pose
  Mat corners; //for undistort image
...
public:
 advancedSubtractBackground( const char* _camera)
    : it_(nh_)
  {
	 thresholdValue = 1;
	...
	sub = n.subscribe(cameraInfoTopic, 1,	 &advancedSubtractBackground::getCameraInfo, this);
	
	//subscribe image topic
    string imageTopic = getImageTopic( camera);
	 cout << imageTopic << endl;
	image_sub_ = it_.subscribe( imageTopic, 1, &advancedSubtractBackground::imageCb, this);
	...
	pub = n.advertise<roscv::ObjectPosition> (objectTopic, 1);
  }
 

  ~advancedSubtractBackground()
  {
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
	...
	///3 line of code for background subtraction
	//Mat imgGray;
	//cvtColor( img1, imgGray, CV_RGB2GRAY);
	//subtractImage = abs( imgGray - avBkgImage); //is it better?
	Mat back;

	//bg.set( "nmixtures", 5);
	///bg.set( "varThreshold", thresholdValue/255);
	//bg.set( "backgroundRatio", thresholdValue/255);
	...
	thresh_callback(thresholdValue, (void*)&img1);
	...
	if (noObject>0)
	{
		...
		//important
		undistortPointsNormalized = computeUndistortCenterandOrientation( corners);
...
//		//msg.data = formatMsgToSend( worldPoints);
		formatMsgAndSend(worldPoints);
//		cout << "Send: "<< msg << endl;
//		pub.publish( msg);
		//getchar();
	}
 ...
 void thresh_callback( int thresh, void* data)  // don't even need this call back, i.e. can be right in ImgCallback

{
	...
	////detect the contours
	findContours (subtractImage, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
	...
	Mat imgBoundingBox= *static_cast<Mat*>(data);
	for (int i = 0; i< noObject; i++)
	{
		...
		for (int j = 0; j< 4; j++)
			line( imgBoundingBox, rect_points[j], rect_points[ (j+1)%4 ], color, 1, 8);
	...
	corners = Mat::zeros( 1,4* noObject, CV_32FC2); //eacho obj 4 corners
	for (int i = 0; i< noObject; i++)
	{
		Point2f rect_points[4];
		objectRect[i].points( rect_points);
		...
		for (int j = 0; j< 4; j++)
			corners.at<Point2f>( 4*i+j) = rect_points[j];
	}


	namedWindow( "advancedSubtractBackground BoundingBox", CV_WINDOW_AUTOSIZE);
		//cvMoveWindow( "BoundingBox", 2*subtractImage.cols, 0);
	imshow("advancedSubtractBackground BoundingBox", imgBoundingBox);
	if (noObject)
		cout << "Distort centers and Points: "<< corners <<endl;
...
Mat computeUndistortCenterandOrientation( Mat distortCenters)
{
	...
	if (noObject >0){
		Mat scaledUndistortCenters = Mat::zeros( distortCenters.rows, distortCenters.cols, CV_32FC2);

		cout << "Before undistort func  \n";
		cout << distortCenters.rows <<" " << distortCenters.cols << endl;
		cout << "Test " << distortCenters << endl;

		undistortPoints( distortCenters, scaledUndistortCenters, K, D); //this is a scaled coordinate

		//scale to image space
		cout << "After undistort func  \n";
...
void getCameraInfo( const sensor_msgs::CameraInfo::ConstPtr& msg)
{
	...
	//grab the cam param
		...
		currentImageWidth = msg->width;
		cout << "Camera Width " << currentImageWidth << endl;
...
{
	imagePointsNormalized.copyTo( worldPoints);//initialize mat
...
string formatMsgAndSend( Mat worldPoints)
{// send by message OjbectPose

	stringstream ss;

	roscv::ObjectPosition msg;
	//msg.data = formatMsgToSend( worldPoints);
	//formatMsgToSend(msg, worldPoints);

	for (int i = 0; i< noObject; i++)
	{
		//get point pair
		//float x0, y0, x1, y1, x2, y2, x3,y3, center, angle;
		Point2f rect_points[4], center(0.0, 0.0);
		for (int j=0; j<4; j++)
		{
			rect_points[j].x = worldPoints.at<double> (0, 4*i+ j);
			center.x += rect_points[j].x/4;
			rect_points[j].y = worldPoints.at<double> (1, 4*i+ j);
			center.y += rect_points[j].y/4;
		}
		...
		float angle;
		angle = atan2f(-rect_points[0].y+ rect_points[majorIndex].y, -rect_points[0].x+ rect_points[majorIndex].x);
		angle *= 180 /pi;
		...
		msg.position.z = Zo;
		msg.theta = angle;
		msg.size.length = l2Distance( rect_points[0], rect_points[majorIndex] );
		msg.size.width = l2Distance( rect_points[0], rect_points[minorIndex] );
		//format output
		//
		cout << "Send: "<< msg << endl;
		pub.publish( msg);
...
int main(int argc, char** argv)
{
	...
	cout << "command: rosrun roscv advancedSubtractBackground left|right|head|kinect \n";

  if ((strcmp( argv[1], "left")) && (strcmp( argv[1], "right")) && (strcmp( argv[1], "head")) && (strcmp( argv[1], "kinect")) )
	  exit(-1);
  ros::init(argc, argv, "advanced_subtract");
  if (ros::names::remap( "image") == "image")
  {
	  ROS_WARN( "command: \t$ ./advancedSubtractBackground image:=<image topic>");
	  //exit(-1);(argc>1) ? argv[1]: "kinect"
  }
  advancedSubtractBackground sB( argv[1]);
  ros::spin();
