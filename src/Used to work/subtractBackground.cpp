...
const double Zo = -.910; //base to the ground
...
class subtractBackground
{
 ...
 ros::Subscriber sub; //for subscribe data
  ...
  Mat undistortCentersNormalized;
...
public:
 subtractBackground( const char* _camera)
    : it_(nh_)
  {
	...
	string cameraInfoTopic = "/cameras/"+ camera+ "_hand_camera/camera_info";
	cout << cameraInfoTopic << endl;
	sub = n.subscribe(cameraInfoTopic, 1,	 &subtractBackground::getCameraInfo, this);
	
	//subscribe image topic
	string imageTopic = "/cameras/"+ camera+ "_hand_camera/camera_info";
		cout << imageTopic << endl;
	image_sub_ = it_.subscribe( imageTopic, 1, &subtractBackground::imageCb, this);
	image_pub_= it_.advertise("outLeft",1);

	//subscribe tf topic
	//sub = n.subscribe("tf", 1, &subtractBackground::getTfInfo, this);
  }
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
	...
	if (noObject>0)
	{
		...
		compute3Dcoordinate( worldPoints, undistortCentersNormalized, R, t);
...
Mat computeUndistortCenters( Mat distortCenters)
{
	...
	if (noObject >0){
		...
		undistortCentersNormalized = Mat::ones( 3, noObject, CV_64F);
		for (int i = 0; i< noObject; i++)
		{
			undistortCentersNormalized.at<double>(0, i) = scaledUndistortCenters.at<Point2f>(0, i).x;
			undistortCentersNormalized.at<double>(1, i) = scaledUndistortCenters.at<Point2f>(0, i).y;
		}
		//
		//cout<< undistortCenters <<endl;
		undistortCenters = K* undistortCentersNormalized;
...
void getTfInfo ()
{
	...
	string frameTo("left_hand_camera");
	double r, p, y;
...
int main(int argc, char** argv)
{
  ...
  {
	  ROS_WARN( "command: \t$ ./subtractBackground image:=<image topic>");
	  //exit(-1);(argc>1) ? argv[1]: "kinect"
  }
  subtractBackground sB( argv[1]);
  ros::spin();
